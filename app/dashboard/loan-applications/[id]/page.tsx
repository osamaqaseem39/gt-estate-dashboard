...
export default function LoanApplicationDetailPage() {
  ...
  const handleDelete = async () => {
    if (!window.confirm('Delete this PM Loan application? This cannot be undone.')) return
    setDeleting(true)
    try {
  ...
  if (isLoading) {
    return <p className="py-12 text-center text-sm text-gray-500">Loading application…</p>
  }
      ...
      <div className="space-y-4 py-12 text-center">
        <p className="text-gray-600">
          {isError
            ...
            : 'PM Loan application not found.'}
        </p>
        <Link href="/dashboard/loan-applications" className={cn(buttonVariants({ variant: 'outline' }))}>
          Back to PM Loan applications
        </Link>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span
            className={`rounded-full px-3 py-1 text-xs font-medium capitalize ${loanStatusClass(application.status)}`}
          >
            {application.status}
          </span>
          <select
